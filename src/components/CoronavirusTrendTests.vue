...
export default {
  ...
  methods: {
    ...
    render() {
      ...
      const linearGradientGreenId = this.createLinearGradient(
        svg,
        'linear-gradient-green',
        '#285e61',
        '#1a202c'
      )
      const linearGradientOrangeId = this.createLinearGradient(
        svg,
        ...
        .on('mousemove', function () {
          ...
          svg
            ...
            .attr('transform', function (_d, _i) {
              ...
              )}, ${y(confirmed[idx].people_tests.daily.total)})`
            })

          svg
            .select('.mouse-per-line-green')
            .attr('transform', function (_d, _i) {
              const xDate = x.invert(mouse[0])
              const bisect = self.$d3.bisector((data) => new Date(data.date))
                .left
              ...
              return `translate(${x(
                new Date(confirmed[idx].date)
              )}, ${y(confirmed[idx].negative_cases.daily.total)})`
            })

          svg
            .select('.mouse-per-line-orange')
            .attr('transform', function (_d, _i) {
              const xDate = x.invert(mouse[0])
              const bisect = self.$d3.bisector((data) => new Date(data.date))
                .left
              ...
              return `translate(${x(
                new Date(confirmed[idx].date)
              )}, ${y(confirmed[idx].positive_cases.daily.total)})`
            })
        })
