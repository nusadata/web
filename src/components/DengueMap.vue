  ...
  >
    <header class="flex flex-wrap items-center">
      <div class="mx-5 lg:mx-0 w-full lg:w-1/2">
        <h2 id="map-of-dengue-fever" class="font-semibold text-2xl pt-4 mb-4">
          {{ $d[locale][`map_of_${currentType}_in`](currentYear) }}
        </h2>
      </div>
      <div class="mb-5 lg:mb-0 flex-none w-full lg:w-1/2">
        <div
          class="mx-5 lg:mx-0 flex items-center justify-start lg:justify-end"
        >
          ...
          <select v-model="currentType" class="text-gray-800">
            <option v-for="type in types" :key="`type-${type}`" :value="type">
              {{ $d[locale][type] }}
            </option>
          </select>
        </div>
...
export default {
  ...
  mounted() {
    this.fetchResources().then(async ([geojson, data]) => {
  ...
  methods: {
    ...
    render(geojson, data) {
      ...
      const generator = this.$d3.geoPath().projection(projection)

      const strokeColor = '#22292f'
      const strokeHoverColor = 'black'

      const svg = this.$d3
        .select(this.getSelector('.content'))
        .append('svg')
        ...
        .attr(
          ...
          (d) =>
            `${data.get(d.properties.slug).name} ${
              data.get(d.properties.slug).value
            }`
        )
    ...
    renderLegend(svg) {
      ...
      linearGradient
        ...
        .attr(
          'offset',
          (d, index) => `${Math.floor(((index + 1) / stops.length) * 100)}%`
        )
      ...
      const textWrapper = legendWrapper
        .append('g')
    ...
    async rerender(year, type) {
      ...
      data.forEach((obj, key) => {
        ...
        try {
          ...
          province.transition().attr('fill', this.color(obj.value))
          document
            .getElementById(id)
            ._tippy.setContent(`${obj.name} ${obj.value}`)
        } catch (e) {
          console.log(key)
        }
      })
    },
    rerenderLegend(_year, _type) {
    ...
    color(value) {
      return this.$d3.scaleQuantize(
        this.colorRange,
        this.$d3.schemeBlues[7]
      )(value)
    },
    async transformToMap(data, type) {
      const parsedCsv = this.$d3.csvParse(await data.text(), (col) => [
        col.slug,
        { value: +col[type], name: col.province },
      ])
