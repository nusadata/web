...
export default {
  ...
  methods: {
    ...
    render() {
      ...
      const linearGradientInvId = this.createLinearGradient(
        svg,
        'linear-gradient-inv',
        '#9c4221',
        '#1a202c'
      )
      const linearGradientSucId = this.createLinearGradient(
        svg,
        'linear-gradient-suc',
        '#285e61',
        '#1a202c'
      ...
      const y = this.$d3
        .scaleLinear()
        .domain([0, this.$d3.max(confirmed, (d) => d.jumlah_positif_kum.value)])
        .range([400, 0])
      ...
      svg
        ...
        .attr(
          ...
          this.$d3
            ...
            .y0(400)
            .y1((d) => y(d.jumlah_positif_kum.value))
      ...
      svg
        ...
        .attr(
          ...
          this.$d3
            .line()
            .x((d) => x(new Date(d.key)))
            .y((d) => y(d.jumlah_positif_kum.value))
            .curve(this.$d3.curveMonotoneX)
        )
      ...
      svg
        ...
        .attr(
          ...
          this.$d3
            ...
            .x((d) => x(new Date(d.key)))
            .y0(400)
            .y1((d) => y(d.jumlah_sembuh_kum.value))
            .curve(this.$d3.curveMonotoneX)
        )
      ...
      svg
        ...
        .attr(
          ...
          this.$d3
            .line()
            .x((d) => x(new Date(d.key)))
            .y((d) => y(d.jumlah_sembuh_kum.value))
            .curve(this.$d3.curveMonotoneX)
        )
      ...
      svg
        ...
        .attr(
          ...
          this.$d3
            ...
            .x((d) => x(new Date(d.key)))
            .y0(400)
            .y1((d) => y(d.jumlah_meninggal_kum.value))
            .curve(this.$d3.curveMonotoneX)
        )
          ...
          svg
            ...
            .attr('transform', function (_d, _i) {
              ...
              self.$d3
                ...
                .text(
                  Math.ceil(
                    y.invert(y(confirmed[idx].jumlah_positif_kum.value))
                  )
                )

              return `translate(${x(
                new Date(confirmed[idx].key)
              )}, ${y(confirmed[idx].jumlah_positif_kum.value)})`
            })

          svg
            .select('.mouse-per-line-recovered')
            .attr('transform', function (_d, _i) {
              const xDate = x.invert(mouse[0])
              const bisect = self.$d3.bisector((data) => new Date(data.key))
                .left
              ...
              self.$d3
                ...
                .select('text')
                .text(
                  Math.ceil(y.invert(y(confirmed[idx].jumlah_sembuh_kum.value)))
                )

              return `translate(${x(
                new Date(confirmed[idx].key)
              )}, ${y(confirmed[idx].jumlah_sembuh_kum.value)})`
            })

          svg
            .select('.mouse-per-line-deaths')
            .attr('transform', function (_d, _i) {
              const xDate = x.invert(mouse[0])
              const bisect = self.$d3.bisector((data) => new Date(data.key))
                .left
              ...
              self.$d3
                ...
                .text(
                  Math.ceil(
                    y.invert(y(confirmed[idx].jumlah_meninggal_kum.value))
                  )
                )

              return `translate(${x(
                new Date(confirmed[idx].key)
              )}, ${y(confirmed[idx].jumlah_meninggal_kum.value)})`
            })
        })
